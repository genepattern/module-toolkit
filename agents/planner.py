...
@planner_agent.tool
def create_implementation_roadmap(context: RunContext[str], tool_name: str, complexity: str = "medium") -> str:
    """
    Create a detailed implementation roadmap for GenePattern module development.
    ...
    """
    print(f"🗺️ PLANNER TOOL: Running create_implementation_roadmap for '{tool_name}' (complexity: {complexity})")
    
    roadmap = f"Implementation Roadmap: {tool_name} GenePattern Module\n"
    roadmap += "=" * 60 + "\n\n"
    
    roadmap += f"**Project Overview:**\n"
    roadmap += f"  - Tool: {tool_name}\n"
    roadmap += f"  - Complexity: {complexity.title()}\n"
    roadmap += f"  - Target: Production-ready GenePattern module\n\n"
    ...
    if complexity.lower() == "simple":
        phases = [
            ("Research & Analysis", 0.5, ["Tool research", "Parameter analysis", "Use case identification"]),
            ("Core Development", 1.5, ["Wrapper script creation", "Basic parameter handling", "Docker container"]),
            ("Integration", 1.0, ["Manifest creation", "Parameter groups", "Documentation"]),
            ("Testing & Validation", 1.0, ["Unit testing", "Integration testing", "User acceptance"])
        ]
    elif complexity.lower() == "medium":
        phases = [
            ("Research & Analysis", 1.0, ["Deep tool research", "Parameter analysis", "Dependency mapping", "Use case study"]),
            ("Architecture Design", 0.5, ["Module architecture", "Data flow design", "Error handling strategy"]),
            ("Core Development", 2.0, ["Wrapper script", "Parameter validation", "Docker optimization", "Output processing"]),
            ("Integration & Configuration", 1.0, ["Manifest creation", "Parameter groups", "Documentation", "Help text"]),
            ("Testing & Quality Assurance", 1.5, ["Unit testing", "Integration testing", "Performance testing", "User testing"])
        ]
    else:  # complex
        phases = [
            ("Research & Analysis", 1.5, ["Comprehensive tool research", "Advanced parameter analysis", "Dependency mapping", "Performance profiling"]),
            ("Architecture Design", 1.0, ["Complex module architecture", "Multi-step workflow design", "Resource management", "Error handling"]),
            ("Core Development", 3.0, ["Advanced wrapper script", "Complex parameter handling", "Multi-stage processing", "Resource optimization"]),
            ("Integration & Configuration", 1.5, ["Complex manifest", "Advanced parameter groups", "Comprehensive documentation", "User guides"]),
            ("Testing & Quality Assurance", 2.0, ["Comprehensive testing", "Performance optimization", "Edge case handling", "Production validation"])
        ]
        
    # Add deliverables and milestones
    roadmap += "**Key Deliverables:**\n"
    roadmap += "  - Functional wrapper script\n"
    roadmap += "  - Optimized Docker container\n"
    roadmap += "  - Complete manifest file\n"
    roadmap += "  - Parameter groups configuration\n"
    roadmap += "  - Comprehensive documentation\n"
    roadmap += "  - Test suite and validation\n\n"
    
    roadmap += "**Quality Gates:**\n"
    roadmap += "  - All parameters properly validated\n"
    roadmap += "  - Docker container builds successfully\n"
    roadmap += "  - Module passes all linter checks\n"
    roadmap += "  - Documentation complete and accurate\n"
    roadmap += "  - Performance meets requirements\n\n"
    
    roadmap += "**Risk Mitigation:**\n"
    roadmap += "  - Regular testing throughout development\n"
    roadmap += "  - Continuous integration setup\n"
    roadmap += "  - Documentation updates with code changes\n"
    roadmap += "  - User feedback integration\n"
    
    print("✅ PLANNER TOOL: create_implementation_roadmap completed successfully")
    return roadmap